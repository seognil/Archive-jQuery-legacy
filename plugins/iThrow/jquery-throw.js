function iThrow(options) {
  ...
  function initStyle() {
    ...
    if (style.size[0][style.size[0].length - 1] == "%") {
      style.size[0] = $(setting.thing).outerWidth() * style.size[0].slice(0, style.size[0].length - 1) / 100 + "px"
    }
    if (style.size[1][style.size[1].length - 1] == "%") {
      style.size[1] = $(setting.thing).outerHeight() * style.size[1].slice(0, style.size[1].length - 1) / 100 + "px"
    }
  ...
  function initDom() {
    dom.$thing = $(setting.thing).clone().css({
      "width": style.size[0],
      "height": style.size[1],
